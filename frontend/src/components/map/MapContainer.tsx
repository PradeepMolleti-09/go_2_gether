...
export const MapContainer = () => {
  ...
  useEffect(() => {
    if (!map || !destination || room?.activeTrip) return; // Hide auto-pan if trip started or search used
    map.panTo({ lat: destination.lat, lng: destination.lng });
    map.setZoom(14);
  }, [map, destination, room?.activeTrip]);
  ...
  useEffect(() => {
    ...
    const watchId = navigator.geolocation.watchPosition(
      (pos) => {
        ...
        if (lastLocationRef.current && tripStats.startTime && !tripStats.endTime) {
          const dist = haversineKm(lastLocationRef.current, coords);
          // Only count if movement is more than 5 meters to filter GPS noise
          if (dist > 0.005) setTripStats((prev: any) => ({ ...prev, distanceTraveled: prev.distanceTraveled + dist }));
        }
